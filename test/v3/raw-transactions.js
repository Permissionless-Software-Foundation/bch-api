...
describe('#Raw-Transactions', () => {
  ...
  describe('decodeRawTransactionSingle()', () => {
    ...
    it('should GET /decodeRawTransaction', async () => {
      ...
      assert.hasAnyKeys(result, ['txid', 'hash', 'size', 'version', 'locktime', 'vin', 'vout'])
  ...
  describe('decodeRawTransactionBulk()', () => {
    ...
    it('should error on non-array single hex', async () => {
      ...
      assert.include(result.error, 'hexes must be an array', 'Proper error message')
    })
    it('returns proper error when downstream service stalls', async () => {
      req.body.hexes = [
        '',
    ...
    it('should decode an array with a single hex', async () => {
      ...
      assert.hasAnyKeys(result[0], ['txid', 'hash', 'size', 'version', 'locktime', 'vin', 'vout'])
  ...
  describe('getRawTransactionBulk()', () => {
    ...
    it('should throw 400 error if txid is invalid', async () => {
      // Mock the RPC call for unit tests.
      if (process.env.TEST === 'unit') {
        sandbox.stub(uut.axios, 'request').rejects('parameter 1 must be of length 64 (not 6)')
      }
    ...
    it('should submit hex encoded transaction', async () => {
      ...
      if (process.env.TEST === 'unit') {
        sandbox.stub(uut.axios, 'request').resolves({
          data: {
            result: 'aef8848396e67532b42008b9d75b5a5a3459a6717740f31f0553b74102b4b118',
          },
        })
      }
  ...
  describe('sendRawTransactionSingle()', () => {
    it('should throw an error for an empty hex', async () => {
      ...
      assert.equal(res.statusCode, 400, 'HTTP status code 400 expected.')
      assert.include(result.error, 'Encountered empty hex', 'Proper error message')
    })
    ...
    it('should throw an error for a non-string', async () => {
      ...
      assert.equal(res.statusCode, 400, 'HTTP status code 400 expected.')
      assert.include(result.error, 'hex must be a string', 'Proper error message')
    })
    ...
    it('should throw 500 when network issues', async () => {
      ...
      assert.isAbove(res.statusCode, 499, 'HTTP status code 500 or great expected.')
    })
